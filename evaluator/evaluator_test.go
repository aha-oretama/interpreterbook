...
import (
	...
	"github.com/aha-oretama/interpreterbook/object"
	"github.com/aha-oretama/interpreterbook/parser"
	"testing"
)
...
func TestBuiltinFunctions(t *testing.T) {
	tests := []struct {
		...
		{`len("four")`, 4},
		{`len("hello world")`, 11},
		{`let fnFour = fn() { "four" }; len(fnFour())`, 4},
	...
	input := `let two = "two";
{
	...
	if len(result.Pairs) != len(expected) {
		t.Fatalf("Hash has wrong number of elements. got=%d", len(result.Pairs))
	}
...
func testErrorObject(t *testing.T, obj object.Object, expected string) bool {
	...
	if !ok {
		t.Fatalf("no error object returned. got=%T (%+v)", obj, obj)
		return false
	}
